...
import { workingAmazonService, RealAmazonProduct } from '../src/lib/working-amazon-service';
...
async function checkUrl(url: string, type: 'Image URL' | 'Product URL'): Promise<boolean> {
  try {
    ...
    console.error(`${colors.red}❌ ERROR${colors.reset}: Checking ${type} for ${url} failed:`, (error as Error).message);
    return false;
  }
}

async function verifyProducts() {
  const products: RealAmazonProduct[] = workingAmazonService.getAllRealProducts();
  ...
  const uniqueImageUrls: Set<string> = new Set(products.map(p => p.imageUrl));
  const uniqueAffiliateUrls: Set<string> = new Set(products.map(p => p.affiliateUrl));
