...
export function SupplementStackCard({ stack, onPurchase, isLoading }: SupplementStackCardProps) {
    ...
    const fetchImageUrls = async () => {
      ...
      for (const supplement of stack.supplements) {
        try {
          ...
          // Try to find by supplement ID pattern (supplement_1, supplement_2, etc.)
          // This matches your Firebase Storage structure
          for (let i = 1; i <= 25; i++) {
            try {
              const firebasePath = `supplement-images/supplement_${i}.jpg`;
              const imageRef = ref(storage, firebasePath);
              const downloadUrl = await getDownloadURL(imageRef);
              urlMap[supplement.name] = downloadUrl;
              console.log(`✅ Found Firebase image by ID for ${supplement.name}: supplement_${i}.jpg`);
              break;
            } catch (idError) {
              // Continue to next ID
              continue;
          ...
          // If still no image found, try Amazon URL as last resort
          if (!urlMap[supplement.name] && supplement?.imageUrl && supplement.imageUrl.startsWith('https://')) {
            urlMap[supplement.name] = supplement.imageUrl;
  ...
  const handleLearnMore = () => {
    console.log('📖 Learn more clicked for stack:', stack.name);
    // Could implement a modal or redirect to detailed page
  };

  const handlePurchase = () => {
    console.log('🛒 Purchase clicked for stack:', stack.name);
    onPurchase(stack);
  };
  ...
  const handleViewProductDetails = (supplementName: string) => {
    ...
    if (supplement?.affiliateUrl) {
      ...
      console.log(`🛒 Affiliate click: ${supplementName}`, { url: supplement?.affiliateUrl ?? 'N/A' });
